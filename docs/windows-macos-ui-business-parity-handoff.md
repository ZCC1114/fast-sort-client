...
- 已按 `LiveRoomsView.swift` 重新拆清 Windows 直播端职责：直播端只保留房间列表、当前直播间、打印机和弹幕面板；WebView2/Cookie 采集与 native adapter 测试继续放在 `DanmakuCookieTestView.xaml`。
- 直播端 `+ 添加直播间` 只作为授权测试/添加流程入口，不再在直播端页面内嵌授权测试区块或复用测试页临时状态。
- `LiveRoomsViewModel.cs` 不再继承 `DanmakuWebAuthViewModelBase`，正式连接只通过后台房间 `liveSession` 交给 `NativeDanmakuSessionCoordinator`。
- 侧栏菜单、底部入口和内容容器已加统一对齐/宽度约束，避免高 DPI 或窄窗口下菜单错位、右侧卡片被长房间号撑出视口。
- Windows 保留原生按钮和下拉交互，不再使用会破坏 `DisplayMemberPath` 的自定义 ComboBox 模板。
