...
仅靠 SwiftUI 源码可以还原结构和业务，但视觉细节仍可能有偏差。为了让 Windows Codex 更接近 100% 复刻，建议从 macOS 客户端截这些页面图，提交到 `docs/parity-screenshots/macos/`：

```text
login.png
dashboard.png
danmaku-auth-douyin.png
danmaku-auth-xhs.png
danmaku-auth-wechat.png
live-rooms.png
entertainment.png
pick.png
blacklist.png
...
Windows Codex 应把截图、macOS 源码、Windows 当前页面三者放在一起对照改。没有截图时，可以先按源码复刻，但不要声称已经达到像素级一致。
