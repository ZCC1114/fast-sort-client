...
{
    private LiveRoomsViewModel? _viewModel;
    private CancellationTokenSource? _autoCollectCts;

    public LiveRoomsView()
    {
        InitializeComponent();
        DataContextChanged += LiveRoomsView_DataContextChanged;
    }
    ...
    private void LiveRoomsView_DataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (_viewModel is not null)
        {
        ...
        _viewModel = e.NewValue as LiveRoomsViewModel;
        if (_viewModel is not null)
        {
            _viewModel.NavigateRequested += ViewModel_NavigateRequested;
        }
